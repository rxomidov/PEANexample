...
export const getAllItems = (id,result) => {
    db.query("SELECT * FROM cart WHERE user_id = $1",[id], (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }else{
...
export const getAItem = (user,food,result) => {
    db.query("SELECT * FROM cart WHERE user_id = $1 AND food_id = $2",[user, food], (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }else{
...
export const insertToCart = (data,result) => {
    const {food_id, item_qty, user_id} = data 
    db.query("INSERT INTO cart (food_id, item_qty, user_id) VALUES ($1, $2, $3)",[food_id, item_qty, user_id], (err,results)=> {
        if (err){
            console.log(err);
            result(err,null);
        }else{
